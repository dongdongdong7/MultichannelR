...
# ****** 0. Initialize ****** #
{
  ...
  # 0.2 Load data
  {
    ...
    dir_path <- "D:/fudan/Projects/2024/MultichannelR/Progress/build_package/test_data/Hy/mix5/"
  ...
  # 0.4 Function
  {
    ...
    GetSpecialFragment <- function(sp2, tolMz = 0.01, num = 10){
      ...
      for(i in 1:nrow(df)){
        Q3 <- df$mz[i]
        logical_Hy <- near(Q3, Q3_Hy, tol = tolMz)
        if(any(logical_Hy)){
          Q3 <- Q3_Hy[which(logical_Hy == TRUE)]
          class <- "Hy"
          break
        }else{
          Q3 <- NA
    ...
    GetSampleIdx <- function(specialFragment){
      ...
      if(!specialFragment %in% Q3){
        ...
        logical_Hy <- specialFragment %in% Q3_Hy
        
        if(logical_Hy){
          sampleIdx <- which(Q3_Hy %in% specialFragment)
        }else{
          sampleIdx <- NA
        }
      }
    ...
    for(i in 1:length(alignedGroupList)){
      ...
      for(j in 1:nrow(alignedGroup_tmp)){
        ...
        for(l in 1:length(missing_channelIdx)){
          ...
          if(missing_class == "Hy"){
            missing_specialFragment <- Q3_Hy[missing_channelIdx_tmp]
          }
...
# ****** 4. Metabolite Identification ****** #
{
  # 4.1 Load database
  {
    HydroxylLibrary <- openxlsx::read.xlsx("D:/fudan/Projects/2024/MultichannelR/Progress/build_package/MultichannelR/Library/HydroxylLibrary.xlsx")
  }
